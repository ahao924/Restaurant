...
public class LoginController extends BaseController{
	...
	public void index() throws Exception{
		...
		String results=HttpUtils.get("https://api.weixin.qq.com/sns/jscode2session?appid=" + business_license.get("appid").toString() + "&secret=" + business_license.get("appsecret").toString() + "&js_code=" + code + "&grant_type=authorization_code");
		...
		if(StrKit.isBlank(errcode)){
			...
			if(user==null){
				user=new User();
				user.set("name", "微信用户")
					.set("img_url", PropKit.get("wxUrl").toString() + "/static/user.png")
					.set("openid", openid)
					.set("business_id", business.get("id"))
					.set("status", User.STATUS_QIYONG)
			...
			session_msg.set("3rd_session", RandomStringUtils.randomAlphanumeric(128))
						.set("openid", openid)
