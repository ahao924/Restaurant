...
public class ApiController extends BaseController{
	...
	public void tables() throws Exception{
		...
		if(StrKit.notBlank(getPara("appointment")) && getParaToInt("appointment")==1){
			Tables tables=Tables.dao.findFirst("select * from db_tables where shop_id=? and system=? and title=?", getLoginUserShoppingShopId(), Tables.SYSTEM_YES, "预约点餐");
			setAttr("tables", tables);
		}else{
			Tables tables=Tables.dao.findFirst("select * from db_tables where shop_id=? and system=? and title=?", getLoginUserShoppingShopId(), Tables.SYSTEM_YES, "外卖点餐");
			setAttr("tables", tables);
		}
		setAttr("code", CodeUtil.OPERATION_SUCCESS);
		setAttr("msg", "操作成功");
