...
public class DishesController extends BaseController{
	...
	public void list() throws Exception{
		...
		for(Record item : results){
			Object month_number=Db.findFirst("select sum(oi.item_number) number from db_orders_item oi left join db_orders o on oi.orders_id=o.id where oi.dishes_id=? and o.status!=? and o.closed=? and o.display=? and o.create_date>=?", item.get("id"), Orders.STATUS_NOT_PAY, Orders.CLOSED_NO, Orders.DISPLAY_YES, DateUtil.monthBefore(new Date(), -1)).get("number");
	...
	public void item() throws Exception{
		...
		for(Record item : dishes_format_list_1){
			...
			for(Record each : dishes_format_list_2){
				List<Record> dishes_format_list_3=DishesFormat.getList3(dishes.get("id"), item.get("title_1"), each.get("title_2"));
