...
public class SynchronizedUtil {
	 ...
	 */
	synchronized
	public static void wxPay(Object orders_id) throws Exception{
						...
						if(orders.getInt("status")==Orders.STATUS_NOT_PAY){
							if(orders.getInt("takeaway")==Orders.TAKEAWAY_YES || orders.getInt("appointment")==Orders.APPOINTMENT_YES){
								...
								orders.set("status", Orders.STATUS_PAY)
										.set("payment", Orders.PAYMENT_XCX)
	 ...
	 */
	synchronized
	public static void wxCharge(Object user_charge_id) throws Exception{
            ...
            public boolean run() throws SQLException {
            	try {
            		...
            		if(user_charge.getInt("status")==UserCharge.STATUS_DAIFUKUAN){
            			user_charge.set("status", UserCharge.STATUS_YIFUKUAN)
            						.update();
            			User user=User.dao.findById(user_charge.get("user_id"));
            			user.set("account", com.project.util.CodeUtil.getNumber(user.getFloat("account") + user_charge.getFloat("account_3")))
            				.update();
            			...
            			user_log.set("code", CodeUtil.getCode())
            					...
            					.set("content", user_charge.get("content"))
            					.set("content_1", user_charge.get("content"))
            					.set("create_date", new Date())
            					.save();
            		}
            	} catch (Exception e) {
	 ...
	 */
	synchronized
	public static void wxRenew(Object business_renew_id) {
            	...
            	try {
					...
					if(business_renew.getInt("status")==BusinessRenew.STATUS_DAIFUKUAN){
						business_renew.set("status", BusinessRenew.STATUS_YIFUKUAN)
										.set("payment", BusinessRenew.PAYMENT_WX)
										.update();
						...
						if(business.getDate("invalid_date").before(new Date())){
							business.set("invalid_date", DateUtil.getStartTimeOfDay(DateUtil.monthBefore(new Date(), business.getInt("month"))))
								.update();
						}else{
							business.set("invalid_date", DateUtil.getStartTimeOfDay(DateUtil.monthBefore(business.getDate("invalid_date"), admin_goods.getInt("month"))))
										.update();
						}
					}
            	} catch (Exception e) {
	 ...
	 */
	synchronized
	public static void alipayRenew(Object business_renew_id) {
		...
		Db.tx(new IAtom() {
            ...
            public boolean run() throws SQLException {
            	try {
					...
					if(business_renew.getInt("status")==BusinessRenew.STATUS_DAIFUKUAN){
						business_renew.set("status", BusinessRenew.STATUS_YIFUKUAN)
										.set("payment", BusinessRenew.PAYMENT_ALIPAY)
										.update();
						Business business=Business.dao.findById(business_renew.get("business_id"));
						AdminGoods admin_goods=AdminGoods.dao.findById(business_renew.get("admin_goods_id"));
						if(business.getDate("invalid_date").before(new Date())){
	 ...
	 */
	synchronized
	public static Map<String, Object> create(Map<String, Object> params, int type, String code) throws Exception{
            ...
            public boolean run() throws SQLException {
            	try {
            		if(type==1){
                		...
                		List<Record> list=ShoppingCart.getByUserShop(params.get("user_id").toString(), params.get("shop_id").toString());
                		float subtotal=0f;
                		...
                		orders.set("code", code)
                				...
                				.set("remark", params.get("remark"))
                				.set("subtotal", subtotal)
                				.set("tableware_price", com.project.util.CodeUtil.getNumber(Integer.parseInt(user_number) * shop.getFloat("tableware_price")))
                				.set("takeaway_price", 0f)
                				.set("tables_price", tables.getFloat("price"))
                				.set("grand_total", com.project.util.CodeUtil.getNumber(subtotal + orders.getFloat("tableware_price") + 0f + tables.getFloat("price") - coupon_saving))
                		...
                		try {
                			Orders.print(orders.get("id").toString(), OrdersItem.getList1(orders.get("id")), ShopPrinter.getList(params.get("shop_id").toString()), "会员下单", orders.getDate("create_date"), 1, params.get("remark"));
                		} catch (Exception e) {
                			e.printStackTrace();
                		}
            			...
            			orders.set("code", code)
            					...
            					.set("tableware_price", com.project.util.CodeUtil.getNumber(Integer.parseInt(user_number) * shop.getFloat("tableware_price")))
            					.set("tables_price", tables.getFloat("price"))
            					.set("grand_total", com.project.util.CodeUtil.getNumber(subtotal + orders.getFloat("tableware_price") + tables.getFloat("price") - coupon_saving))
            					.set("display", Orders.DISPLAY_YES)
            					.set("status", Orders.STATUS_NOT_PAY)
            					.set("create_date", new Date())
            					.save();
            			...
            			try {
            				Orders.print(orders.get("id").toString(), OrdersItem.getList1(orders.get("id")), ShopPrinter.getList(params.get("shop_id").toString()), "门店代客下单", orders.getDate("create_date"), 1, params.get("remark"));
            			} catch (Exception e) {
            				e.printStackTrace();
            			}
            			results.put("success", true);
