...
public class Business extends Model<Business> {
	...
	public static Business getByTelephone(String telephone) {
		...
		return Business.dao.findFirst("select * from db_business where telephone=? and status=?", telephone, Business.STATUS_ENABLE);
	}
	/**
	 * 
	 * 
	...
	public static String getAccessToken(String business_id) throws Exception{
		...
		if((new Date().getTime() - business_authorizer.getDate("create_date").getTime()) / 1000 < 6000){
			if(business_authorizer.get("access_token")!=null && StrKit.notBlank(business_authorizer.get("access_token").toString())){
        ...
        HttpRequest request=HttpRequest.post("https://api.weixin.qq.com/cgi-bin/component/api_authorizer_token?component_access_token=" + access_token)
		...
		business_authorizer.set("access_token", authorizer_access_token)
							.set("refresh_token", authorizer_refresh_token)
