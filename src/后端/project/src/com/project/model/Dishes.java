...
public class Dishes extends Model<Dishes> {
	...
	public static List<Record> getListByBusiness(Object business_id, Object dishes_type_id) {
		...
		if(dishes_type_id!=null && StrKit.notBlank(dishes_type_id.toString())){
			return Db.find("select * from db_dishes where (status=? or status=?) and display=? and dishes_type_id=? and business_id=? and shop_id is null order by create_date asc", Dishes.STATUS_XIAOSHOU, Dishes.STATUS_SHOUQING, Dishes.DISPLAY_YES, dishes_type_id, business_id);
		}
		return Db.find("select * from db_dishes where (status=? or status=?) and display=? and business_id=? and shop_id is null order by create_date asc", Dishes.STATUS_XIAOSHOU, Dishes.STATUS_SHOUQING, Dishes.DISPLAY_YES, business_id);
	}
	/**
	 * 
	 * 
	...
	public static List<Record> getListByShop(Object shop_id, Object dishes_type_id) {
		...
		if(dishes_type_id!=null && StrKit.notBlank(dishes_type_id.toString())){
			return Db.find("select * from db_dishes where (status=? or status=?) and display=? and dishes_type_id=? and shop_id=? order by create_date asc", Dishes.STATUS_XIAOSHOU, Dishes.STATUS_SHOUQING, Dishes.DISPLAY_YES, dishes_type_id, shop_id);
		}
		return Db.find("select * from db_dishes where (status=? or status=?) and display=? and shop_id=? order by create_date asc", Dishes.STATUS_XIAOSHOU, Dishes.STATUS_SHOUQING, Dishes.DISPLAY_YES, shop_id);
	}
	/**
	 * 
	 * 
	...
	public static List<Dishes> getByParent(Object parent_dishes_id) {
		...
		return Dishes.dao.find("select * from db_dishes where display=? and parent_dishes_id=?", Dishes.DISPLAY_YES, parent_dishes_id);
	}
}