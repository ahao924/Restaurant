...
public class WxCodeJob implements Runnable {
	...
	public void run() {
		...
		List<Business> list=Business.dao.find("select * from db_business where authorize_status=? and (code_status=? or code_status=?) and status=?", Business.AUTHORIZE_STATUS_YISHOUQUAN, Business.CODE_STATUS_AUDIT, Business.CODE_STATUS_SHIBAI, Business.STATUS_ENABLE);
		for(Business item : list){
			try {
				if(item.get("code_auditid")!=null && StrKit.notBlank(item.get("code_auditid").toString())){
				    ...
				    HttpRequest request=HttpRequest.post("https://api.weixin.qq.com/wxa/get_auditstatus?access_token=" + access_token)
					...
					if(jsonObject.getInteger("errcode")==0){
						...
						//0为审核成功，1为审核失败，2为审核中
						item.set("code_status", jsonObject.get("status"))
							.set("code_reason", jsonObject.get("reason"))
							.update();
						if(jsonObject.getInteger("status")==Business.CODE_STATUS_CHENGGONG){
							body=new JSONObject();
							access_token=Business.getAccessToken(item.get("id").toString());
					        request=HttpRequest.post("https://api.weixin.qq.com/wxa/release?access_token=" + access_token)
												.header("Content-Type", "application/json;charset=utf-8")
												.send(body.toString());
							res=request.body();
							request.disconnect();
