...
public class OrderController extends BaseController{
	...
	public void tables() throws Exception{
		...
		user.set("shop_id", tables.get("shop_id"))
			.update();
	...
	public void addShopping() throws Exception{
		...
		ShoppingCart shopping_cart=ShoppingCart.getByUserDishesFormat(getLoginUserId(), dishes.get("id"), dishes_format);
		if(shopping_cart!=null){
			...
			shopping_cart.set("number", getParaToInt("number") + shopping_cart.getInt("number"))
							.update();
		}else{
			...
			shopping_cart.set("dishes_id", dishes.get("id"))
							.set("dishes_format_title_1", dishes_format.get("title_1"))
							.set("dishes_format_title_2", dishes_format.get("title_2"))
							.set("dishes_format_title_3", dishes_format.get("title_3"))
							.set("number", getParaToInt("number"))
							.set("user_id", getLoginUserId())
							.set("shop_id", getLoginUserShoppingShopId())
	...
	public void deletedAll() throws Exception{
		...
		Db.update("delete from db_shopping_cart where user_id=? and shop_id=?", getLoginUserId(), getLoginUserShoppingShopId());
		setAttr("code", CodeUtil.OPERATION_SUCCESS);
		setAttr("msg", "操作成功");
		renderJson();
		return;
	...
	public void changeNumber() throws Exception{
		...
		DishesFormat dishes_format=DishesFormat.getByDishesTitle(dishes.get("id"), shopping_cart.get("dishes_format_title_1"), shopping_cart.get("dishes_format_title_2"), shopping_cart.get("dishes_format_title_3"));
		...
		shopping_cart.set("number", number)
						.update();
		setAttr("code", CodeUtil.OPERATION_SUCCESS);
		setAttr("msg", "操作成功");
		renderJson();
	...
	public void createTakeaway() throws Exception{
		...
		if(StrKit.notBlank(getPara("appointment")) && getParaToInt("appointment")==1){
			//预约点餐
			orders.set("takeaway", Orders.TAKEAWAY_NO)
					.set("appointment", Orders.APPOINTMENT_YES);
		}else{
			orders.set("takeaway", Orders.TAKEAWAY_YES);
		}
		orders.set("code", com.project.util.CodeUtil.getCode())
				.set("wx_code", com.project.util.CodeUtil.getCode())
				.set("small_code", ((int)(Math.random() * 9000) + 1000))
				.set("user_number", user_number)
				.set("tables_id", tables.get("id"))
				.set("user_id", getLoginUserId())
				.set("shop_id", getLoginUserShoppingShopId())
				.set("business_id", getLoginUserBusinessId())
				.set("remark", getPara("remark"))
				.set("subtotal", subtotal)
				.set("tables_price", 0f)
				.set("take_name", take_name)
				.set("take_mobile", take_mobile)
				.set("take_address", take_address)
				.set("take_date", take_date)
				.set("closed_date", DateUtil.getAfterMiniterByCount(new Date(), 15))
		...
		if(StrKit.notBlank(getPara("appointment")) && getParaToInt("appointment")==1){
			...
			orders.set("takeaway_price", 0f)
					.set("tableware_price", com.project.util.CodeUtil.getNumber(Integer.parseInt(user_number) * shop.getFloat("tableware_price")))
					.set("grand_total", com.project.util.CodeUtil.getNumber(subtotal + com.project.util.CodeUtil.getNumber(Integer.parseInt(user_number) * shop.getFloat("tableware_price")) + 0f + 0f - coupon_saving));
		}else{
			if(getParaToInt("take_own")==Orders.TAKE_OWN_NO){
				log="外卖下单（配送）：";
				orders.set("takeaway_price", shop.getFloat("takeaway_price"))
						.set("take_own", Orders.TAKE_OWN_NO)
						.set("tableware_price", 0f)
						.set("grand_total", com.project.util.CodeUtil.getNumber(subtotal + 0f + orders.getFloat("takeaway_price") + 0f - coupon_saving));
				//起送金额
				...
				String distance=AddressUtil.driving(map_1.get("lat").toString() + "," + map_1.get("lng").toString(), shop.get("lat").toString() + "," + shop.get("lng").toString());
				...
				orders.set("distance", distance)
						.set("lng", map_1.get("lng"))
						.set("lat", map_1.get("lat"));
			}else if(getParaToInt("take_own")==Orders.TAKE_OWN_YES){
				log="外卖下单（自提）：";
				orders.set("takeaway_price", 0f)
						.set("take_own", Orders.TAKE_OWN_YES)
						.set("tableware_price", 0f)
						.set("grand_total", com.project.util.CodeUtil.getNumber(subtotal + 0f + 0f + 0f - coupon_saving));
			}
		}
		orders.save();
		for(Record item : list){
	...
	public void wxPay() throws Exception{
		...
		if(orders.getInt("takeaway")==Orders.TAKEAWAY_YES || orders.getInt("appointment")==Orders.APPOINTMENT_YES){
			//外卖或者预约订单
			if(orders.getInt("status")!=Orders.STATUS_NOT_PAY){
				setAttr("code", CodeUtil.OPERATION_FAILED);
		...
		orders.set("wx_code", com.project.util.CodeUtil.getCode())
				.update();
		User user=User.dao.findById(getLoginUserId());
		...
		if(orders.getInt("status")!=Orders.STATUS_PAY){
			...
			if(business.getInt("fuwushang")==Business.FUWUSHANG_DISABLE){
				...
				xmlParam = "<xml>"
								...
								+ "<out_trade_no>" + orders.get("wx_code").toString() + "</out_trade_no>"
							+ "</xml>";
			}else{
				String appid = business_license.get("appid").toString();
				String mch_id = business_license.get("partner").toString();
	...
	public void accountPay() throws Exception{
		...
		if(orders.getInt("takeaway")==Orders.TAKEAWAY_YES || orders.getInt("appointment")==Orders.APPOINTMENT_YES){
			...
			if(orders.getInt("appointment")==Orders.APPOINTMENT_YES){
				Orders.print(orders.get("id").toString(), OrdersItem.getList1(orders.get("id")), ShopPrinter.getList(orders.get("shop_id")), "预约下单", orders.getDate("create_date"), 1, orders.get("remark"));
			}else{
				if(orders.getInt("take_own")==Orders.TAKE_OWN_NO){
					Orders.print(orders.get("id").toString(), OrdersItem.getList1(orders.get("id")), ShopPrinter.getList(orders.get("shop_id")), "外卖下单|配送", orders.getDate("create_date"), 1, orders.get("remark"));
				}else if(orders.getInt("take_own")==Orders.TAKE_OWN_YES){
					Orders.print(orders.get("id").toString(), OrdersItem.getList1(orders.get("id")), ShopPrinter.getList(orders.get("shop_id")), "外卖下单|自提", orders.getDate("create_date"), 1, orders.get("remark"));
				}
			}
			List<Record> item_list=OrdersItem.getList1(orders.get("id"));
			for(Record item : item_list){
		...
		user_log.set("code", com.project.util.CodeUtil.getCode())
