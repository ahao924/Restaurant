...
public class YuyuezhuoweiController extends BaseController{
	...
	public void save() throws Exception{
		...
		yuyuezhuowei.set("code", com.project.util.CodeUtil.getCode())
					...
					.set("status", Yuyuezhuowei.STATUS_DAICHULI)
	...
	public void user() throws Exception{
		...
		List<Record> list=Db.find("select y.*,tt.title tables_type_title from db_yuyuezhuowei y left join db_tables_type tt on y.tables_type_id=tt.id where y.user_id=? and y.shop_id=? order by y.create_date desc", getLoginUserId(), getLoginUserShoppingShopId());
		setAttr("list", list);
		//是否存在待处理，预约成功
		Record yuyuezhuowei=Db.findFirst("select y.*,tt.title tables_type_title from db_yuyuezhuowei y left join db_tables_type tt on y.tables_type_id=tt.id where y.user_id=? and y.shop_id=? and (y.status=? or y.status=?) order by y.create_date desc", getLoginUserId(), getLoginUserShoppingShopId(), Yuyuezhuowei.STATUS_DAICHULI, Yuyuezhuowei.STATUS_YUYUECHENGGONG);
		setAttr("yuyuezhuowei", yuyuezhuowei);
		setAttr("code", CodeUtil.OPERATION_SUCCESS);
		setAttr("msg", "操作成功");
		renderJson();
	...
	public void deleted() throws Exception{
		...
		yuyuezhuowei.set("status", Yuyuezhuowei.STATUS_YIQUXIAO)
					.update();
		setAttr("code", CodeUtil.OPERATION_SUCCESS);
		setAttr("msg", "操作成功");
		renderJson();
