...
public class UsersOrdersController extends BaseController{
	...
	public void jiacan() throws Exception{
		...
		if(!orders.get("user_id").toString().equals(getLoginUserId()) || orders.getInt("takeaway")==Orders.TAKEAWAY_YES){
			setAttr("code", CodeUtil.OPERATION_FAILED);
			setAttr("msg", "没有操作权限");
			renderJson();
			return;
		...
		for(Coupon coupon : coupon_list){
			if(com.project.util.CodeUtil.getNumber(orders.getFloat("subtotal") + subtotal) >= coupon.getFloat("total_account")){
		...
		orders.set("subtotal", com.project.util.CodeUtil.getNumber(orders.getFloat("subtotal") + subtotal))
				.set("grand_total", com.project.util.CodeUtil.getNumber(orders.getFloat("subtotal") + orders.getFloat("tableware_price") + orders.getFloat("tables_price") - orders.getFloat("coupon_saving")))
				.update();
		...
		orders_log.set("orders_id", orders.get("id"))
					...
					.save();
		try {
			Orders.print(orders.get("id").toString(), print_list, ShopPrinter.getList(getLoginUserShoppingShopId()), "会员加餐", new Date(), 0, getPara("remark"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		setAttr("code", CodeUtil.OPERATION_SUCCESS);
