...
public class ChargeController extends BaseController{
	...
	public void index() throws Exception{
		...
		if (StrKit.notBlank(getPara("bid")) && StrKit.isBlank(getPara("startT")) && StrKit.isBlank(getPara("endT"))){
			//累计充值金额
			Object account = Db.findFirst("select ifnull(sum(account_1),0) number from db_user_charge where status=? and business_id=?", UserCharge.STATUS_YIFUKUAN, getPara("bid")).get("number");
			setAttr("account", CodeUtil.getNumber(Float.parseFloat(account.toString())));
			//累计充值笔数
			Object number = Db.findFirst("select count(id) number from db_user_charge where status=? and business_id=?", UserCharge.STATUS_YIFUKUAN, getPara("bid")).get("number");
			setAttr("number", number);
		}else if(StrKit.notBlank(getPara("bid")) && StrKit.notBlank(getPara("startT")) && StrKit.notBlank(getPara("endT"))){
			//累计充值金额
			Object account = Db.findFirst("select ifnull(sum(account_1),0) number from db_user_charge where status=? and business_id=? and create_date>=? and create_date<=?", UserCharge.STATUS_YIFUKUAN, getPara("bid"), DateUtil.formatDate(getParaToDate("startT"), "yyyy-MM-dd HH:mm"), DateUtil.formatDate(getParaToDate("endT"), "yyyy-MM-dd HH:mm")).get("number");
			setAttr("account", CodeUtil.getNumber(Float.parseFloat(account.toString())));
			//累计充值笔数
			Object number = Db.findFirst("select count(id) number from db_user_charge where status=? and business_id=? and create_date>=? and create_date<=?", UserCharge.STATUS_YIFUKUAN, getPara("bid"), DateUtil.formatDate(getParaToDate("startT"), "yyyy-MM-dd HH:mm"), DateUtil.formatDate(getParaToDate("endT"), "yyyy-MM-dd HH:mm")).get("number");
			setAttr("number", number);
		}else if(StrKit.isBlank(getPara("bid"))&& StrKit.notBlank(getPara("startT")) && StrKit.notBlank(getPara("endT"))){
			//累计充值金额
			Object account = Db.findFirst("select ifnull(sum(account_1),0) number from db_user_charge where status=? and create_date>=? and create_date<=?", UserCharge.STATUS_YIFUKUAN, DateUtil.formatDate(getParaToDate("startT"), "yyyy-MM-dd HH:mm"), DateUtil.formatDate(getParaToDate("endT"), "yyyy-MM-dd HH:mm")).get("number");
			setAttr("account", CodeUtil.getNumber(Float.parseFloat(account.toString())));
			//累计充值笔数
			Object number = Db.findFirst("select count(id) number from db_user_charge where status=? and create_date>=? and create_date<=?", UserCharge.STATUS_YIFUKUAN, DateUtil.formatDate(getParaToDate("startT"), "yyyy-MM-dd HH:mm"), DateUtil.formatDate(getParaToDate("endT"), "yyyy-MM-dd HH:mm")).get("number");
			setAttr("number", number);
		}else {
			//累计充值金额
			Object account = Db.findFirst("select ifnull(sum(account_1),0) number from db_user_charge where status=?", UserCharge.STATUS_YIFUKUAN).get("number");
			setAttr("account", CodeUtil.getNumber(Float.parseFloat(account.toString())));
			//累计充值笔数
			Object number = Db.findFirst("select count(id) number from db_user_charge where status=?", UserCharge.STATUS_YIFUKUAN).get("number");
			setAttr("number", number);
		}
		render("list.htm");
	}
