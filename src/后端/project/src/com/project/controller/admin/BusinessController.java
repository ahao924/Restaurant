...
public class BusinessController extends BaseController{
	...
	public void changeStatus() throws Exception{
		...
		business.set("status", getPara("status"))
				.set("shenhe", Business.SHENHE_YISHENHE)
				.update();
	...
	public void shenhechehui() throws Exception{
		...
		String res=HttpKit.get("https://api.weixin.qq.com/wxa/undocodeaudit?access_token=" + business_authorizer.get("access_token"));
		JSONObject jsonObject=JSON.parseObject(res);
		System.out.println(jsonObject);
		if(jsonObject.getInteger("errcode")==0){
			Business business=Business.dao.findById(getPara("id"));
			business.set("code_status", null)
	    			.set("code_reason", null)
	    			.set("code_version", null)
	    			.set("code_auditid", null)
	    			.update();
			setAttr("success", true);
			setAttr("msg", "操作成功");
			renderJson();
	...
	public void tijiaoshenhe() throws Exception{
		...
		HttpRequest request=HttpRequest.post("https://api.weixin.qq.com/wxa/modify_domain?access_token=" + access_token)
		...
		if(jsonObject.getInteger("errcode")==0){
			...
			if(jsonObject.getInteger("errcode")==0){
				...
				if(jsonObject.getInteger("errcode")==0){
			        ...
			        request=HttpRequest.post("https://api.weixin.qq.com/wxa/submit_audit?access_token=" + access_token)
										.header("Content-Type", "application/json;charset=utf-8")
										.send(body.toString());
					res=request.body();
					request.disconnect();
					...
					if(jsonObject.getInteger("errcode")==0){
						...
						business.set("code_status", Business.CODE_STATUS_AUDIT)
								...
								.set("code_auditid", jsonObject.getInteger("auditid"))
								.update();
						try {
							body=new JSONObject();
							request=HttpRequest.post("https://api.weixin.qq.com/wxa/queryquota?access_token=" + access_token)
									.header("Content-Type", "application/json;charset=utf-8")
									.send(body.toString());
							res=request.body();
							request.disconnect();
							...
							System.out.println("当月提审剩余额：" + jsonObject.getInteger("rest") + "，" +  "当月提审限额：" + jsonObject.getInteger("limit") + "，" +  "剩余加急次数：" + jsonObject.getInteger("speedup_rest") + "，" +  "当月分配加急次数：" + jsonObject.getInteger("speedup_limit"));
						} catch (Exception e) {
							e.printStackTrace();
						}
						setAttr("success", true);
	...
	public void changeFuwushang() throws Exception{
		...
		Business business=Business.dao.findById(getPara("id"));
		business.set("fuwushang", getPara("fuwushang"))
				.update();
		setAttr("success", true);
		setAttr("msg", "操作成功");
		renderJson();
	...
	public void updatePwd() throws Exception{
		...
		BusinessAdmin business_admin = BusinessAdmin.dao.findFirst("select * from db_business_admin where business_id=? and type=?", business.get("id"), BusinessAdmin.TYPE_1);
