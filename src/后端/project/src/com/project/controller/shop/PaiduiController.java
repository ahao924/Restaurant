...
public class PaiduiController extends BaseController{
	...
	public void changeStatus() throws Exception{
		...
		paidui.set("status", getPara("status"))
		...
		try {
			if(getParaToInt("status")==Paidui.STATUS_YIJIAOHAO){
				if(paidui.get("user_id")!=null && StrKit.notBlank(paidui.get("user_id").toString())){
					...
					TemplateUtil.paidui(params, user.get("openid").toString(), Business.getAccessToken(shop.get("business_id").toString()));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void quhao() throws Exception{
		...
		Object number=Paidui.dao.findFirst("select count(id) number from db_paidui where shop_id=? and create_date>=?", getLoginShopId(), DateUtil.getStartTimeOfDay(new Date())).get("number");
	...
	public void changePaidui() throws Exception{
		...
		Shop shop=Shop.dao.findById(getLoginShopId());
		shop.set("paidui", getPara("paidui"))
			.update();
		setAttr("success", true);
		setAttr("msg", "操作成功");
		renderJson();
