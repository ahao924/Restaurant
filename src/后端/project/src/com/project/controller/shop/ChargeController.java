...
public class ChargeController extends BaseController{
	...
	public void save() throws Exception{
		...
		if(StrKit.notBlank(getPara("content"))){
			user_log.set("content", "门店|" + getLoginShopAdminName() + "（" + getLoginShopAdminId() + "）" + "充值" + account + "，备注：" + getPara("content"));
			user_log.set("content_1", "门店" + getLoginShop().get("title") + "线下充值" + account);
		}else{
			user_log.set("content", "门店|" + getLoginShopAdminName() + "（" + getLoginShopAdminId() + "）" + "充值" + account + "，备注：无");
			user_log.set("content_1", "门店" + getLoginShop().get("title") + "线下充值" + account);
		}
		user_log.set("code", CodeUtil.getCode())
				.set("user_id", user.get("id"))
				.set("account", Float.parseFloat(account))
	...
	public void update() throws Exception{
		...
		UserLog user_log=new UserLog();
		if(StrKit.notBlank(getPara("content"))){
			user_log.set("content", "门店|" + getLoginShopAdminName() + "（" + getLoginShopAdminId() + "）" + "扣费" + account + "，备注：" + getPara("content"));
			user_log.set("content_1", "门店" + getLoginShop().get("title") + "线下扣费" + account);
		}else{
			user_log.set("content", "门店|" + getLoginShopAdminName() + "（" + getLoginShopAdminId() + "）" + "扣费" + account + "，备注：无");
			user_log.set("content_1", "门店" + getLoginShop().get("title") + "线下扣费" + account);
		}
		user_log.set("code", CodeUtil.getCode())
				.set("user_id", user.get("id"))
