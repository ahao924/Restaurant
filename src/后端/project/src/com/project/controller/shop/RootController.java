...
public class RootController extends BaseController{
	...
	public void save() throws Exception{
		...
		shop_admin.set("name", name)
					...
					.set("type", ShopAdmin.TYPE_2)
					.set("create_date", new Date())
					.save();
		for (String smid : smids) {
			ShopAdminMenu shop_admin_menu = new ShopAdminMenu();
			shop_admin_menu.set("shop_admin_id", shop_admin.get("id"))
							.set("shop_menu_id", smid)
							.set("create_date", new Date())
							.save();
		}
	...
	public void edit() throws Exception{
		...
		if(!getLoginShopId().equals(shop_admin.get("shop_id").toString())|| getLoginShopAdmin().getInt("type")!=ShopAdmin.TYPE_1){
			setAttr("msg", "没有操作权限");
			render("/shop/msg.htm");
			return;
		}
		...
		for (Record item : menu_list) {
			ShopAdminMenu shop_admin_menu = ShopAdminMenu.dao.findFirst("select * from db_shop_admin_menu where shop_admin_id=? and shop_menu_id=?",shop_admin.get("id"), item.get("id"));
	...
	public void deleted() throws Exception{
		...
		shop_admin.set("status", ShopAdmin.STATUS_DELETED)
					.update();
		setAttr("success", true);
		setAttr("msg", "操作成功");
		renderJson();
