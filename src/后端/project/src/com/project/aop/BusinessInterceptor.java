...
public class BusinessInterceptor implements Interceptor{
	...
	public void intercept(Invocation ai) {
    	...
    	if (ai.getActionKey().startsWith("/business")) {
    		...
    		if (controller.getLoginBusiness() == null || controller.getLoginBusinessAdmin() == null || controller.getSessionAttr("business_admin_menu") == null) {
					...
					if (business_admin != null && business_admin.getInt("status") == BusinessAdmin.STATUS_ENABLE) {
						...
						if (business != null && business.getInt("status") == Business.STATUS_ENABLE) {
							...
							if(business_admin_menu!=null && business_admin_menu.size()>0){
								...
								controller.setCookie("business_email", business_admin.get("email").toString(), 60 * 60 * 24 * 30);
								controller.setCookie("business_password", business_admin.get("password").toString(), 60 * 60 * 24 * 30);
			...
			if(business.getDate("invalid_date").before(new Date()) && !ai.getActionKey().equals("/business/dashboard") && !ai.getControllerKey().equals("/business/goods")){
