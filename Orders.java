...
public class Orders extends Model<Orders> {
	 ...
	 * 技术QQ：2511251392
	 */
	public static void print(String orders_id, List<Record> list, List<Record> printer_list, String title, Date create_date, int type, Object remark) throws Exception{
		...
		content+="--------------------------------<BR>";
		List<Record> type_list=Db.find("select * from db_dishes_type where business_id=? order by idx asc", shop.get("business_id").toString());
		...
		if(type==1){
			...
			if(orders.getFloat("coupon_saving")!=0){
				content+="优  惠：￥" + orders.get("coupon_saving").toString() + "（" + orders.get("coupon_title").toString() + "）" + "<BR>";
			}
		...
		for(Record printer : printer_list){
			...
			nvps.add(new BasicNameValuePair("sig", signature(printer.get("user_code").toString(), printer.get("user_pwd").toString(), stime)));
